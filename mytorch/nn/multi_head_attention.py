...
class MultiHeadAttention:
    ...
    def _merge_masks(self, key_padding_mask, attn_mask):
        ...
        key_mask = (
            ...
            if key_padding_mask is not None else np.zeros((self.N, 1, 1, self.S), dtype=bool)
        )
        attention_mask = (
            attn_mask[None, None, :, :]
            if attn_mask is not None else np.zeros((1, 1, self.L, self.S), dtype=bool)
        )
    ...
    def _split_heads(self, x):
        ...
        x = x.reshape(self.N, self.L, self.num_heads, self.head_dim)
