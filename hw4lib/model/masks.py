...
def PadMask(padded_input, input_lengths):
    ...
    device = padded_input.device   # Ensure mask is on the same device

    # Create a range tensor (1D) and expand it to (N, T)
    range_tensor = torch.arange(T, device=device).expand(N, T)

    # Create the mask by comparing the range tensor with input lengths
...
def CausalMask(padded_input):
    ...
    _, T = padded_input.shape[:2]  # Get sequence length
    device = padded_input.device   # Ensure mask is on the same device
