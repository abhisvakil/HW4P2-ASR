    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        for i, layer in enumerate(self.dec_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            x, attention = layer(x, pad_mask_dec, causal_mask)
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        print(max_len)
        print(layer_drop_rate)
        print(num_classes)
        print(num_encoder_layers)
        print(num_decoder_layers)
        print(skip_encoder_pe)
        print(skip_decoder_pe)
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        batch_size, max_len = x_enc.size(0), x_enc.size(1)
        device = x_enc.device
        pad_mask_src = torch.arange(max_len, device=device).expand(batch_size, max_len) >= x_enc_lengths.unsqueeze(1)
        ...
        for i, layer in enumerate(self.enc_layers):
            ...
            x_enc, attn_weights = layer(x_enc, pad_mask_src)
        
            # Save attention weights
            running_att[f'layer{i+1}_enc_self'] = attn_weights
    ...
    def decode(
        ...
        batch_size, tgt_len = padded_targets.size()

        # TODO: Create target padding mask on the same device as the input
        pad_mask_tgt = None
        if target_lengths is not None:
            #pad_mask_tgt = NotImplementedError
            pad_mask_tgt = torch.arange(tgt_len, device=device).expand(batch_size, tgt_len) >= target_lengths.unsqueeze(1)
