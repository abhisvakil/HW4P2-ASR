...
class ASRDataset(Dataset):
    def __init__(
        ...
        files = [f for f in os.listdir(self.fbank_dir) if os.path.isfile(os.path.join(self.fbank_dir, f))]
        self.fbank_files = sorted(files)
        
        # TODO: Take subset
        #subset_size      = NotImplementedError
        #self.fbank_files = NotImplementedError
        #self.fbank_files = self.fbank_files[:10]
        ...
        if self.partition != "test-clean":
            ...
            files = [f for f in os.listdir(self.text_dir) if os.path.isfile(os.path.join(self.text_dir, f))]
            self.text_files = sorted(files)
            
            # TODO: Take subset
            #self.text_files = NotImplementedError
            #self.text_files = self.text_files[:10]
        ...
        for i in tqdm(range(self.length)):
            ...
            feat = torch.from_numpy(np.load(feat_path)).float().T  # Shape (num_feats, time)
    ...
    def collate_fn(self, batch) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        feat_lengths = torch.tensor([f.shape[1] for f in feats])  # (B,)
        ...
        padded_feats = pad_sequence([f.T for f in feats], batch_first=True, padding_value=0.0)  # (B, T, F)
        ...
        if self.partition != "test-clean":
            ...
            batch_shifted = [torch.tensor(s) if not isinstance(s, torch.Tensor) else s for s in shifted]
            batch_golden = [torch.tensor(g) if not isinstance(g, torch.Tensor) else g for g in golden]
            ...
            transcript_lengths = torch.tensor([g.size(0) for g in batch_golden])  # (B,)
            ...
            padded_shifted = pad_sequence(batch_shifted, batch_first=True, padding_value=self.pad_token)  # (B, T)
            #padded_golden      = NotImplementedError # B x T
            padded_golden = pad_sequence(batch_golden, batch_first=True, padding_value=self.pad_token)    # (B, T)
        ...
        if self.config["specaug"] and self.isTrainPartition:
            ...
            if self.config["specaug_conf"]["apply_freq_mask"]:
                for _ in range(self.config["specaug_conf"]["num_freq_mask"]):
                    ...
                    max_width = self.config["specaug_conf"].get("freq_mask_width_range", 0)
                    width = random.randint(0, max_width)
                    freq_mask = tat.FrequencyMasking(freq_mask_param=width)
                    padded_feats = freq_mask(padded_feats)

            # TODO: Apply time masking
            if self.config["specaug_conf"]["apply_time_mask"]:
                for _ in range(self.config["specaug_conf"]["num_time_mask"]):
                    #padded_feats = NotImplementedError
                    max_width = self.config["specaug_conf"].get("time_mask_width_range", 0)
                    width = random.randint(0, max_width)
                    time_mask = tat.TimeMasking(time_mask_param=width)
                    padded_feats = time_mask(padded_feats)
