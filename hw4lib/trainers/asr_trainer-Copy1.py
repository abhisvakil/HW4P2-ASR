...
class ASRTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        ignore_index = self.tokenizer.pad_id  # Typically pad token is the index to ignore in CE
        ...
        self.ctc_loss_fn = nn.CTCLoss(blank=self.tokenizer.pad_id, zero_infinity=True)
    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            #     #ignore_index=self.config['model']['pad_index']
            ...
            if self.ctc_weight > 0:
                ...
                #log_probs = ctc_inputs.log_softmax(dim=-1).transpose(0, 1)  # [T, B, C]
                #input_lengths = feat_lengths // 4  # Assumes 4x subsampling
                #log_probs = F.log_softmax(ctc_inputs['log_probs'], dim=-1).transpose(0, 1)
                #ctc_loss = self.ctc_loss_fn(ctc_inputs['log_probs'], targets_golden, input_lengths, transcript_lengths)
    ...
    def _validate_epoch(self, dataloader):
        ...
        mlen = dataloader.dataset.text_max_len
        print(mlen)
        results = self.recognize(dataloader,max_length=mlen)
    ...
    def recognize(self, dataloader, recognition_config: Optional[Dict[str, Any]] = None, config_name: Optional[str] = None, max_length: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        with torch.inference_mode():
            for i, batch in enumerate(dataloader):
                ...
                sos_token = self.tokenizer.sos_id
                prompts = torch.full((batch_size, 1), sos_token, dtype=torch.long, device=self.device)
                ...
                if targets_golden is not None:
                    ...
                    for j, (pred, target) in enumerate(zip(post_processed_preds, post_processed_targets)):
                        t = self.tokenizer.decode(target.tolist(), skip_special_tokens=True)
                        g = self.tokenizer.decode(pred.tolist(), skip_special_tokens=True)
                        results.append({
                            'target': t,
                            'generated': g,
                            'score': scores[j].item()
