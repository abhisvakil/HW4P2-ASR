...
class LMTrainer(BaseTrainer):
    ...
    def __init__(self, model, tokenizer, config, run_name, config_file, device=None):
        ...
        label_smoothing = config.get("loss", {}).get("label_smoothing", 1.0)
        self.criterion = nn.CrossEntropyLoss(label_smoothing=label_smoothing, ignore_index=-100)
    ...
    def _train_epoch(self, dataloader) -> Tuple[Dict[str, float], Dict[str, torch.Tensor]]:
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                raw_preds = raw_preds.view(-1, raw_preds.size(-1))        # [B*T, vocab]
                targets_golden = targets_golden.view(-1)                  # [B*T]
